...
int evaluate(int ply = 0)
{
    ...
    while (pawns)
    {
        ...
        int p_score = piece_square[0][side_to_move == Color::WHITE ? square : square ^ 56] * turn;
        positional_score += p_score;
    }
    while (knights)
    {
        int square = knights.pop();
        int n_score = piece_square[1][side_to_move == Color::WHITE ? square : square ^ 56] * turn;
        positional_score += n_score;
    }
    while (bishops)
    {
        int square = bishops.pop();
        int b_score = piece_square[2][side_to_move == Color::WHITE ? square : square ^ 56] * turn;
        positional_score += b_score;
    }
    while (rooks)
    {
        int square = rooks.pop();
        int r_score = piece_square[3][side_to_move == Color::WHITE ? square : square ^ 56] * turn;
        positional_score += r_score;
    }
    while (queens)
    {
        int square = queens.pop();
        int q_score = piece_square[4][side_to_move == Color::WHITE ? square : square ^ 56] * turn;
